      ...
      <table>
        ...
        while($row=mysqli_fetch_array($result)){
          ...
          $_SESSION['Product_to_finalize']=$row['auctionId'];
					...
					$query="SELECT OrderId from orders inner join product on currBid=amount
					 																	 where auctionId='$oid';";
					$result=mysqli_query($db,$query) or die("Η σύνδεση απέτυχε");
					$orid=mysqli_fetch_all($result);
					$rid=$orid[0][0];
					$_SESSION["bidid"]=$rid;
        ...
        mysqli_close($db);
        ?>
        <button type='submit' name='submit' value='16'>Finalize</button>
 </body>
</html>
